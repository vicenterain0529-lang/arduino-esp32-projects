...
// ═══════════════════════════════════════════════════════════
//  MEDICINE DISPENSER v4.1 — TRAPDOOR ANTI-WASTE FIX
// ═══════════════════════════════════════════════════════════

// ─── HARDWARE PINS ─────────────────────────────────────────
#define IR_PIN              2
#define SERVO_PIN           9
#define TRAPDOOR_SERVO_PIN  10
#define RED_LED             5
#define GREEN_LED           6
#define BUZZER_PIN          7
...
void openTrapdoor() {
  if(!trapdoorOpen) {
    trapdoorServo.write(TRAPDOOR_OPEN);
    trapdoorOpen = true;
  }
}
...
// ═══════════════════════════════════════════════════════════
//  ALERT STATE MACHINE — TRAPDOOR FIX
// ═══════════════════════════════════════════════════════════
void triggerAlert(uint8_t idx) {
  ...
  // Move wheel to progressive position and STAY there
  uint8_t servoPos = (idx + 1) * 30;   // Dose1=30°, Dose2=60°, ..., Dose6=180°
  servo.write(servoPos);

  // Keep trapdoor CLOSED — medicine stays in compartment
  closeTrapdoor();
...
void confirmDose() {
  ...
  // Close trapdoor after dispensing
  closeTrapdoor();
  ...
  // Only reset wheel servo to 0° if ALL doses for the day are completed
  bool allDone = true;
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
...
void missedDose() {
  ...
  // Close trapdoor — medicine stays in compartment for next cycle
  closeTrapdoor();
...
void resetSystem() {
  ...
  popupEnd = 0;
  cmdEnd   = 0;
  closeTrapdoor();  // Ensure trapdoor is closed
  lcd.clear();
}
...
void doSnooze() {
  ...
  // Close trapdoor if user snoozes — medicine stays put
  closeTrapdoor();
...
void updateAlert() {
  ...
  bool handPresent = (readDistance() < 4);

  // ─── TRAPDOOR LOGIC ─────────────────────────────────────
  // Hand detected and trapdoor is closed → OPEN trapdoor, drop medicine
  if(handPresent && !trapdoorOpen) {
    openTrapdoor();
    lcdPopupF(F("Dispensing..."), F("Catch medicine"), 1500);
    return;
  }

  // Trapdoor is open and hand removed → medicine caught, confirm taken
  if(trapdoorOpen && !handPresent) {
    confirmDose();
    return;
  }
  // ────────────────────────────────────────────────────────
...
void handleCommand(char* raw) {
  ...
  if(strcmp_P(raw, PSTR("ok")) == 0 || strcmp_P(raw, PSTR("yes")) == 0) {
    if(state == ST_ALERT_SOFT || state == ST_ALERT_URGENT) {
      openTrapdoor();      // Manual confirm: open trapdoor to drop medicine
      confirmDose();
    }
...
void setup() {
  ...
  lcd.setCursor(0,1); lcd.print(F("   v4.1+GATE"));
  ...
  btPln(F("\r\n================================"));
  btPln(F("  MEDICINE DISPENSER v4.1+GATE"));
  btPln(F("================================"));
  Serial.print(F("Time: "));
  btPrintTime(now.hour(), now.minute());
  Serial.println();
  ...
  lcd.setCursor(0,1); lcd.write(LCD_PILL); lcd.print(F(" Gate Active"));
...
void loop() {
  ...
  if(now.day() != lastDay) {
    ...
    servo.write(SERVO_CLOSED);
    closeTrapdoor();   // Ensure trapdoor is closed on new day
  }
