/*
  ============================================================
  SMARTFARM v4.2 -- RAIN-AWARE INTELLIGENT EDITION (DIGITAL)
...
#define LDR_PIN      9
#define SERVO_PIN    10
#define RAIN_PIN     11  // NEW: Digital rain sensor input
#define SOIL1_PIN    A0
#define SOIL2_PIN    A1
#define SOIL3_PIN    A2
#define GAS_PIN      A3
...
#define ON           LOW
#define OFF          HIGH
#define EEPROM_MAGIC 161  // Bumped for new config
...
bool isRaining = false;               // Rain detection state
bool rainDetected = false;            // Latched rain event for popup
bool lastRainState = false;           // For edge detection
uint32_t rainStartMs = 0;             // When rain started
uint32_t rainClearMs = 0;             // When rain cleared
const uint32_t RAIN_DEBOUNCE_MS = 2000;  // 2 second debounce
const uint32_t RAIN_POPUP_COOLDOWN = 300000; // 5 min between rain popups (300000ms)

// -- TIMING VARIABLES ----------------------------------------
uint32_t lastSensorMs=0, lastLcdMs=0, p1StartMs=0, p2StartMs=0, lastServoMs=0, lastDebugMs=0;
uint32_t lastRainMs=0;
uint8_t currServoPos=90, tgtServoPos=90, lcdPage=0, dynPage=255;
...
// NEW: Read and process digital rain sensor
void updateRainSensor() {
  // Read digital rain sensor (LOW = rain detected, HIGH = dry)
  bool rainNow = (digitalRead(RAIN_PIN) == LOW);
  uint32_t now = millis();
  
  // Edge detection with debounce
  if (rainNow && !lastRainState) {
    // Potential rain start - start debounce
    ...
    // Reset debounce timers if state is stable
    if (!rainNow) {
      rainStartMs = 0;
      lastRainState = false;
    }
  ...
  // Show rain detected popup (with cooldown)
...
void updateServo() {
  // MODIFIED: Rain overrides everything and forces 90 degrees (closed)
  int goal;
  if (isRaining) {
    goal = 90;  // Rain detected - close roof immediately
  } else {
    goal = emergencyState ? 180 : (lightBright ? 90 : 180);
  }
...
void showPage(uint8_t pg) {
  ...
  switch(pg) {
    ...
    case 1:
      ...
      lcd.write(223);
      // MODIFIED: Added Light (D/N) and Rain (1/0) status
      lcd.print(F(" "));
...
void setup() {
  ...
  // NEW: Digital rain sensor with internal pull-up
