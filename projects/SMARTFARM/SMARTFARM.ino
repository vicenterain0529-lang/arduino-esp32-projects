/*
  ============================================================
  SMARTFARM v4.1 — INTELLIGENT UNIFIED EDITION
...
// ── CORE ENGINES (PRESERVED + ENHANCED) ─────────────────────
...
void updateServo() {
  int goal = emergencyState ? 180 : (lightBright ? 90 : 180);
...
void printHelp() {
  BT.println();
  BT.println(F("=== SMARTFARM v4.1 COMMANDS ==="));
...
void setup() {
  ...
  lcd.setCursor(0,0);
  lcd.print(F("SMARTFARM v4.1"));
  lcd.setCursor(0,1);
  lcd.print(F("INITIALIZING..."));
  pTone(1);
  delay(800);

  const char* checks[] = {"RTC", "DHT", "RELAYS", "SERVO", "EEPROM"};
