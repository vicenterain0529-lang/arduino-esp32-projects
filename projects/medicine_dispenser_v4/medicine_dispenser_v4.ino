...
// Pre-alert tracking (runtime only, resets daily)
...
void dfPlay(int track) {
  if(!dfReady) return;
  dfSerial.listen();       // temporarily let DFPlayer receive
  dfPlayer.stop();
  delay(80);
  dfPlayer.playMp3Folder(track);
  delay(100);
  BT.listen();             // give control back to Bluetooth
}
...
void checkSchedule() {
  ...
  // ─── 10-minute pre-alert voice reminder ────────────────
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
...
// ═══════════════════════════════════════════════════════════
//  SETUP — FIXED DFPlayer boot timing
// ═══════════════════════════════════════════════════════════
void setup() {
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
    doses[i].hour = -1;
  ...
  // ═══ DFPlayer Mini Init — WITH SD CARD MOUNT DELAY ═══
  dfSerial.begin(9600);

  // CRITICAL: DFPlayer Mini needs 2-3 seconds after serial start
  // for its internal microcontroller to boot and mount the SD card.
  // Without this delay, all play commands are silently ignored.
  lcd.clear();
  lcd.setCursor(0,0); lcd.print(F("DFPlayer Init..."));
  delay(3000); // <-- SD card mount wait
  ...
  if(dfPlayer.begin(dfSerial, false)) {
    ...
    // Now that SD is mounted, play boot message
    dfPlay(7); // 0007.mp3 — Boot announcement
    delay(2500); // Let the voice line finish
  } else {
    dfReady = false;
  ...
  // Restore Bluetooth as active SoftwareSerial listener
  BT.listen();
