/*
  ...
  - Roof opens automatically after rain stops (3-min hysteresis)
...
// Rain hysteresis for roof control
uint32_t lastRainMs = 0;
const uint32_t RAIN_HYSTERESIS_MS = 180000; // 3 minutes
...
void updateSensors(bool force) {
  ...
  // Fixed: Check both temperature and humidity
...
// -- SERVO (Faster + Improved Rain Logic) -------------------
void updateServo() {
  int goal;
  if (isRaining) {
    goal = 90;
    ...
    if ((millis() - lastRainMs) < RAIN_HYSTERESIS_MS) {
      goal = 90;   // Stay closed shortly after rain
    } else {
  ...
  if ((int32_t)(now - lastServoMs) > 20L) {
    ...
    if (currServoPos < goal) {
      currServoPos = min(180, currServoPos + 4);  // Faster movement
    } else {
...
void showPage(uint8_t pg) {
  ...
  switch(pg) {
    case 0: // LIVE DATA with Rain + Light status
      lcd.setCursor(0,0); 
      lcd.write(2);
      lcd.print(pct[0]);
      ...
      // Added: Rain status and Photoresistor (D/N)
...
void setup() {
  ...
  // Sensor array initialization (Fix #3)
