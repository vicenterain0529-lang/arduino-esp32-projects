...
// ═══════════════════════════════════════════════════════════
//  MEDICINE DISPENSER v4.0 — DFPLAYER VOICE EDITION
// ═══════════════════════════════════════════════════════════

// ─── HARDWARE PINS ─────────────────────────────────────────
#define BT_RX        10
#define BT_TX        11
#define IR_PIN       2      // IR sensor input
#define SERVO_PIN    9
#define RED_LED      5
#define GREEN_LED    6
#define BUZZER_PIN   7

// NEW: DFPlayer Mini pins (replacing removed buttons)
#define DF_RX        4      // Connects to DFPlayer TX
#define DF_TX        3      // Connects to DFPlayer RX (use 1kΩ series resistor)
#define DF_BUSY      8      // LOW when module is playing audio

// ─── DFPLAYER VOICE TRACK MAP (/mp3/ folder) ───────────────
// 0001.mp3 — Time for your medicine dose (triggerAlert)
// 0002.mp3 — Your medicine is due in 10 minutes (pre-alert)
// 0003.mp3 — Dose taken, great job (confirmDose)
// 0004.mp3 — You missed your medicine dose (missedDose)
// 0005.mp3 — Snooze activated for 10 minutes (doSnooze)
// 0006.mp3 — Please take your medicine now, urgently (escalation)
// 0007.mp3 — Medicine dispenser ready (boot)
...
#define CMD_MS        1000
#define SOFT_TIMEOUT  30000UL   // 30s before escalate
#define HARD_TIMEOUT  60000UL   // 60s before missed
#define SNOOZE_MS     600000UL  // 10 min
...
const uint8_t CHAR_BAR0[8]  PROGMEM = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
SoftwareSerial     dfSerial(DF_RX, DF_TX);   // DFPlayer comms
...
bool    dfReady     = false;                 // True if DFPlayer initialized

// NEW: Pre-alert tracking (runtime only, resets daily)
...
void dfPlay(int track) {
  ...
  delay(80);
  dfPlayer.playMp3Folder(track);   // Plays /mp3/0001.mp3, etc.
  delay(100);
}

bool isDfBusy() {
  return digitalRead(DF_BUSY) == LOW;
}

// ═══════════════════════════════════════════════════════════
//  AUDIO — Buzzer tones (kept alongside voice)
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
//  BLUETOOTH — Identical functions
// ═══════════════════════════════════════════════════════════
void btPln(const __FlashStringHelper* s) { BT.println(s); }
void btPrt(const __FlashStringHelper* s) { BT.print(s); }
void btPln(const char* s)               { BT.println(s); }
void btPrt(const char* s)               { BT.print(s); }

void btPrintTime(uint8_t h, uint8_t m) {
  if(h < 10) BT.print('0');
  BT.print(h); BT.print(':');
  if(m < 10) BT.print('0');
  BT.print(m);
...
void btHelp() {
  btPln(F("\r\n======= COMMANDS ======="));
  btPln(F(" setN HH:MM  Set dose N (1-6)"));
  btPln(F("   e.g: set1 08:30"));
  ...
  btPln(F(" snooze / z  Delay alert 10min"));
...
// ═══════════════════════════════════════════════════════════
//  LCD UI — Preserved exactly (no menus, no buttons)
// ═══════════════════════════════════════════════════════════
void lcdTimePrint(uint8_t h, uint8_t m) {
  if(h < 10) lcd.print('0');
...
// Master LCD updater — preserved exactly
void updateLCD() {
...
// ═══════════════════════════════════════════════════════════
//  ALERT STATE MACHINE — Voice prompts added
// ═══════════════════════════════════════════════════════════
void triggerAlert(uint8_t idx) {
  ...
  uint8_t pos = 30 + idx * 30;
  servo.write(pos);
  ledWarn();

  btPln(F("\r\n**** ALERT ****"));
  BT.print(F("Dose ")); BT.print(idx+1); BT.print(F("  "));
  btPrintTime(doses[idx].hour, doses[idx].minute);
  BT.println();
  btPln(F("Hold hand near IR  OR  send 'ok'"));
  btPln(F("Send 'snooze' to delay 10min"));
  btPln(F("***************"));

  dfPlay(1);      // 0001.mp3 — Time for your medicine
  sndAlert1();
}
...
void confirmDose() {
  ...
  dfPlay(3);      // 0003.mp3 — Dose taken, great job
  sndSuccess();
  delay(3000);
  resetSystem();
}
...
void missedDose() {
  ...
  btPrt(F("\r\n>>>> MISSED: Dose "));
  BT.println(curDose+1);

  dfPlay(4);      // 0004.mp3 — You missed your dose
  sndMissed();
  delay(3000);
  resetSystem();
}
...
void doSnooze() {
  ...
  lcdPopupF(F("  SNOOZED  "), F("Resume in 10min"));
  dfPlay(5);      // 0005.mp3 — Snooze activated
  sndSnooze();
}
...
void checkSchedule() {
  if(state == ST_SNOOZED) {
    if(millis() >= snoozeEnd) {
      ...
      alertStart = millis();
      ledWarn();
      dfPlay(1);  // Re-alert medicine time
      sndAlert1();
    }
    return;
  }
  ...
  if(debugMode) {
    BT.print(F("[CHECK] ")); BT.println(hm);
  }

  // ─── NEW: 10-minute pre-alert voice reminder ────────────
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
    ...
    if(hm == warnTime) {
      preAlerted[i] = true;
      dfPlay(2);   // 0002.mp3 — Medicine in 10 minutes
      lcdPopupF(F("UPCOMING DOSE"), F("In 10 minutes"));
      BT.print(F("[PRE-ALERT] Dose ")); BT.print(i+1);
      BT.print(F(" at ")); btPrintTime(doses[i].hour, doses[i].minute);
      btPln(F(" — 10 min warning"));
    }
  }

  // ─── Standard on-the-dot alert check ────────────────────
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
    if(doses[i].hour < 0)        continue;
    if(doses[i].taken != 0)      continue;
  ...
  switch(state) {
    case ST_ALERT_SOFT:
      if(elapsed >= SOFT_TIMEOUT) {
        ...
        dfPlay(6);      // 0006.mp3 — Take medicine NOW
        sndAlert2();
      } else {
...
// ═══════════════════════════════════════════════════════════
//  COMMAND PROCESSOR — Identical to working code
// ═══════════════════════════════════════════════════════════
...
void handleCommand(char* raw) {
  ...
  if(strcmp_P(raw, PSTR("page")) == 0 || strcmp_P(raw, PSTR("p")) == 0) {
    lcdPage = (lcdPage + 1) % 5;
    BT.print(F("Page ")); BT.println(lcdPage);
    lcd.setCursor(0,1);
    lcd.print(F("Page ")); lcd.print(lcdPage);
    sndChirp();
    return;
  }
  ...
  if(strcmp_P(raw, PSTR("debug")) == 0 || strcmp_P(raw, PSTR("d")) == 0) {
    debugMode = !debugMode;
    BT.print(F("Debug: ")); btPln(debugMode ? F("ON") : F("OFF"));
    sndChirp();
    return;
  }
  ...
  if(strncmp_P(raw, PSTR("set"), 3) == 0 && isdigit(raw[3])) {
    ...
    if(h > 23 || m > 59) {
      btPln(F("ERROR: Bad time. Use HH:MM (24h)"));
      lcdPopupF(F("BAD TIME"), F("Use HH:MM 24h"));
      sndError(); return;
    }
    ...
    BT.print(F("Dose ")); BT.print(idx+1);
    BT.print(F(" set to ")); btPrintTime(h, m); BT.println();

    char l1[17], l2[17];
    snprintf(l1, sizeof(l1), "Dose %d SET", idx+1);
    snprintf(l2, sizeof(l2), "%02d:%02d saved", h, m);
    lcdPopup(l1, l2);
    sndSuccess();
    return;
  }
  ...
  if(strncmp_P(raw, PSTR("del"), 3) == 0 && isdigit(raw[3])) {
    ...
    if(idx < MAX_DOSES) {
      ...
      doses[idx].timeVal = 0;
      saveData();
      BT.print(F("Dose ")); BT.print(idx+1); btPln(F(" deleted"));
      char l1[17];
      snprintf(l1, sizeof(l1), "Dose %d DELETED", idx+1);
      lcdPopup(l1, "");
      sndDismiss();
    } else {
  ...
  if(strncmp_P(raw, PSTR("take"), 4) == 0 && isdigit(raw[4])) {
    ...
    if(idx < MAX_DOSES && doses[idx].hour >= 0) {
      if(state != ST_IDLE) {
        btPln(F("ERROR: Alert already active"));
        sndError(); return;
      }
      triggerAlert(idx);
    } else {
      btPln(F("ERROR: Dose not set"));
      lcdPopupF(F("NOT SET"), F("Schedule it first"));
      sndError();
    }
    return;
  }
  ...
  if(strncmp_P(raw, PSTR("skip"), 4) == 0 && isdigit(raw[4])) {
    ...
    if(idx < MAX_DOSES && doses[idx].hour >= 0) {
      doses[idx].taken = 3;
      saveData();
      BT.print(F("Dose ")); BT.print(idx+1); btPln(F(" skipped today"));
      char l1[17];
      snprintf(l1, sizeof(l1), "Dose %d SKIPPED", idx+1);
      lcdPopup(l1, "Will reset tmrw");
      sndDismiss();
    } else {
      btPln(F("ERROR: Dose not set or out of range"));
      sndError();
    }
    return;
  }
...
// ═══════════════════════════════════════════════════════════
//  SETUP — DFPlayer added, buttons removed
// ═══════════════════════════════════════════════════════════
void setup() {
  for(uint8_t i = 0; i < MAX_DOSES; i++) {
    doses[i].hour = -1;
  ...
  if(dfPlayer.begin(dfSerial, false)) {
    ...
    btPln(F("[DF] Voice module ready"));
    dfPlay(7);                     // 0007.mp3 — Boot announcement
  } else {
    dfReady = false;
    btPln(F("[DF] Voice module NOT detected"));
  }
...
// ═══════════════════════════════════════════════════════════
//  MAIN LOOP — Buttons removed, voice logic runs freely
// ═══════════════════════════════════════════════════════════
void loop() {
  processBT();
  ...
  if(now.day() != lastDay) {
    ...
    for(uint8_t i = 0; i < MAX_DOSES; i++) {
      if(doses[i].taken != 1) doses[i].taken = 0;
      preAlerted[i] = false;       // Reset 10-min warnings for new day
    }
    saveData();
